...
__webpack_require__.d(__webpack_exports__, {
  "default": function() { return /* binding */ makeup_menu_button_dist_mjs_src_default; }
});
...
;// CONCATENATED MODULE: ./packages/makeup-exit-emitter/node_modules/makeup-next-id/dist/mjs/index.js
const mjs_sequenceMap = {};
const mjs_defaultPrefix = "nid";
const mjs_randomPortion = mjs_createRandomPortion(3);
function mjs_randomNumber(max) {
  return Math.floor(Math.random() * max);
}
function mjs_createRandomPortion(size) {
  const letters = "abcdefghijklmnopqrstuvwxyz";
  const digits = "0123456789";
  const allChars = letters + digits;
  let portion = letters[mjs_randomNumber(25)];
  for (let i = 1; i < size; i++) {
    portion += allChars[mjs_randomNumber(35)];
  }
  return portion;
}
function src_default(el, prefix = mjs_defaultPrefix) {
  const separator = prefix === "" ? "" : "-";
  const key = `${prefix}${separator}${mjs_randomPortion}`;
  mjs_sequenceMap[key] = mjs_sequenceMap[key] || 0;
  if (!el.id) {
    el.setAttribute("id", `${key}-${mjs_sequenceMap[key]++}`);
  }
  return el.id;
}
...
function dist_mjs_src_default(el, keyboardOnly = false, callback) {
  if (callback) {
...
function manageFocus(focusManagement, contentEl) {
  if (focusManagement === "content") {
    ...
    dist_mjs_src_default(contentEl)[0].focus();
  } else if (focusManagement === "interactive") {
    dist_mjs_src_default(contentEl, true)[0].focus();
...
class makeup_expander_dist_mjs_src_default {
...
;// CONCATENATED MODULE: ./packages/makeup-navigation-emitter/node_modules/makeup-key-emitter/dist/mjs/index.js
...
;// CONCATENATED MODULE: ./packages/makeup-navigation-emitter/node_modules/makeup-next-id/dist/mjs/index.js
const dist_mjs_sequenceMap = {};
const dist_mjs_defaultPrefix = "nid";
const dist_mjs_randomPortion = dist_mjs_createRandomPortion(3);
function dist_mjs_randomNumber(max) {
  return Math.floor(Math.random() * max);
}
function dist_mjs_createRandomPortion(size) {
  const letters = "abcdefghijklmnopqrstuvwxyz";
  const digits = "0123456789";
  const allChars = letters + digits;
  let portion = letters[dist_mjs_randomNumber(25)];
  for (let i = 1; i < size; i++) {
    portion += allChars[dist_mjs_randomNumber(35)];
  }
  return portion;
}
function makeup_next_id_dist_mjs_src_default(el, prefix = dist_mjs_defaultPrefix) {
  const separator = prefix === "" ? "" : "-";
  const key = `${prefix}${separator}${dist_mjs_randomPortion}`;
  dist_mjs_sequenceMap[key] = dist_mjs_sequenceMap[key] || 0;
  if (!el.id) {
    el.setAttribute("id", `${key}-${dist_mjs_sequenceMap[key]++}`);
  }
  return el.id;
}


;// CONCATENATED MODULE: ./packages/makeup-navigation-emitter/node_modules/makeup-exit-emitter/dist/mjs/index.js

const mjs_focusExitEmitters = {};
function mjs_doFocusExit(el, fromElement, toElement) {
  el.dispatchEvent(new CustomEvent("focusExit", {
    detail: { fromElement, toElement },
    bubbles: false
  }));
}
function mjs_onDocumentFocusIn(e) {
  const newFocusElement = e.target;
  const targetIsDescendant = this.el.contains(newFocusElement);
  if (targetIsDescendant === true) {
    this.currentFocusElement = newFocusElement;
    ...
    window.removeEventListener("blur", this.onWindowBlurListener);
    document.removeEventListener("focusin", this.onDocumentFocusInListener);
    mjs_doFocusExit(this.el, this.currentFocusElement, newFocusElement);
    this.currentFocusElement = null;
  }
}
function mjs_onWindowBlur() {
  mjs_doFocusExit(this.el, this.currentFocusElement, void 0);
}
function mjs_onWidgetFocusIn() {
  document.addEventListener("focusin", this.onDocumentFocusInListener);
  window.addEventListener("blur", this.onWindowBlurListener);
}
class mjs_FocusExitEmitter {
  constructor(el) {
    this.el = el;
    this.currentFocusElement = null;
    this.onWidgetFocusInListener = mjs_onWidgetFocusIn.bind(this);
    this.onDocumentFocusInListener = mjs_onDocumentFocusIn.bind(this);
    this.onWindowBlurListener = mjs_onWindowBlur.bind(this);
    this.el.addEventListener("focusin", this.onWidgetFocusInListener);
  }
  removeEventListeners() {
    window.removeEventListener("blur", this.onWindowBlurListener);
...
function mjs_addFocusExit(el) {
  let exitEmitter = null;
  makeup_next_id_dist_mjs_src_default(el);
  if (!mjs_focusExitEmitters[el.id]) {
    exitEmitter = new mjs_FocusExitEmitter(el);
    mjs_focusExitEmitters[el.id] = exitEmitter;
  }
  return exitEmitter;
}
function mjs_removeFocusExit(el) {
  const exitEmitter = mjs_focusExitEmitters[el.id];
  if (exitEmitter) {
    exitEmitter.removeEventListeners();
    delete mjs_focusExitEmitters[el.id];
  }
}
...
;// CONCATENATED MODULE: ./packages/makeup-menu/dist/mjs/index.js


const makeup_menu_dist_mjs_defaultOptions = {
  customElementMode: false
};
class makeup_menu_dist_mjs_src_default {
  constructor(widgetEl, selectedOptions) {
    this._options = Object.assign({}, makeup_menu_dist_mjs_defaultOptions, selectedOptions);
    this.el = widgetEl;
    this._rovingTabIndex = mjs_createLinear(this.el, "[role^=menuitem]", {
      autoReset: 0
    });
    mjs_add(this.el);
...
;// CONCATENATED MODULE: ./packages/makeup-menu-button/dist/mjs/index.js


const makeup_menu_button_dist_mjs_defaultOptions = {
  customElementMode: false,
  expandedClass: "menu-button--expanded",
  menuSelector: ".menu-button__menu",
  buttonTextSelector: `.expand-btn__text`
};
class makeup_menu_button_dist_mjs_src_default {
  constructor(widgetEl, selectedOptions) {
    this._options = Object.assign({}, makeup_menu_button_dist_mjs_defaultOptions, selectedOptions);
    this.el = widgetEl;
    this._buttonEl = widgetEl.querySelector("button");
    this.menu = new makeup_menu_dist_mjs_src_default(widgetEl.querySelector(this._options.menuSelector));
    this._buttonPrefix = this._buttonEl.dataset?.makeupMenuButtonPrefix;
    this._buttonTextEl = this._buttonEl.querySelector(makeup_menu_button_dist_mjs_defaultOptions.buttonTextSelector);
    this._expander = new makeup_expander_dist_mjs_src_default(widgetEl, {
...
var _makeupMenuButton = _interopRequireDefault(__webpack_require__(4900));
...
window.onload = function () {
  document.querySelectorAll('.menu-button').forEach(function (el, i) {
    ...
    widget.menu.el.addEventListener('makeup-menu-button-mutation', function (e) {
      return console.log(e.type, e.detail);
    });
  });
};
