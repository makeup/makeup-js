...
describe("given an element", () => {
  ...
  test("when module imported should not be undefined", function () {
    expect(add).not.toBe(undefined);
    expect(remove).not.toBe(undefined);
  });

  test("should prevent default for key codes 32 to 40", () => {
  ...
  test("should not prevent default for key codes outside 32 to 40", () => {
    const preventDefaultMock = vi.fn();
  ...
  test("should remove the event listener correctly", () => {
    const preventDefaultMock = vi.fn();
